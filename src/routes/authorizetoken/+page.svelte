<script>
    ...
    onMount(async () => {
        const response = await fetch("https://wbrk-anime-api.cyclic.app/authorize?code="+authorizationtoken, {credentials: "include"});
        if (response.status === 200) {
            console.log(response);
            let json = await response.json();
            if (json.succesfull) {
                let userinfoR = await fetch("https://wbrk-anime-api.cyclic.app/user/info", {credentials: "include"});
